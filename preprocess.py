    ...
    with open(filename, "r") as fh:
        ...
        for article in tqdm(source["data"]):
            for para in article["paragraphs"]:
                ...
                spans = convert_idx(context, context_tokens)  # Character span of each token
                    ...
                    for answer in qa["answers"]:
                        ...
                        for idx, span in enumerate(spans):
                            if not (answer_end <= span[0] or answer_start >= span[1]):
                                # Think of this as "If current span is within question answer"
                                answer_span.append(idx)
                        y1, y2 = answer_span[0], answer_span[-1]  # Id of starting and ending token within context
                        y1s.append(y1)
                        y2s.append(y2)
...
def build_features(args, examples, data_type, out_file, word2idx_dict, char2idx_dict, is_test=False):
    ...
    for n, example in tqdm(enumerate(examples)):
        ...
        if is_answerable(example):
            start, end = example["y1s"][-1], example["y2s"][-1]  # NOTE: Gets the last answer only
        else:
            start, end = -1, -1
    ...
    np.savez(out_file,
             context_idxs=np.array(context_idxs),  # Tensor containing vectors for each context. Each vector contains
                                                   # ids of tokens within the context.
             context_char_idxs=np.array(context_char_idxs),  # Tensor of matrices for each context. Each matrix is
                                                             # composed of vectors containing ids of characters for each
                                                             # token.
             ques_idxs=np.array(ques_idxs),  # Similar to context_idxs
             ques_char_idxs=np.array(ques_char_idxs),  # Similar to context_char_idxs
             y1s=np.array(y1s),  # Id of starting tokens within context
             y2s=np.array(y2s),  # Id of ending tokens within context
             ids=np.array(ids))  # Example id
...
if __name__ == '__main__':
    ...
    # Preprocess dataset
    args_.train_file = args_.train_data_file
    args_.dev_file = args_.dev_data_file
    if args_.include_test_examples:
        args_.test_file = args_.test_data_file
    glove_dir = args_.glove_file  # url_to_data_path(args_.glove_url.replace('.zip', ''))
